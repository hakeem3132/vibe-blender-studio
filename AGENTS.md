...
## Vibe Blender Studio Product Overlay

The current product is **Vibe Blender Studio 0.1.0-dev**, built on the pinned
blender-ai-mcp v3.3.0 foundation. Normal users interact through a beginner
Blender-native prompt workflow; they must not need Python, MCP, RPC or schema
knowledge.

- Never execute unrestricted model-generated `exec` or `eval`.
- Validate every AI action as a typed, versioned ChangeSet and execute only
  allowlisted Blender handlers on Blender's main thread.
- Every supported edit is transactional, verifiable, rejectable and undoable;
  micro-prompts preserve unrelated state.
- Stable UUIDs are authoritative; Blender names are display labels.
- Deterministic scene inspection is authoritative over visual/model opinion.
- Never claim mock tests are real Blender tests or launch readiness while real
  acceptance gates are blocked.
- Keep unfinished later-milestone controls disabled or absent from release UI.
- Package license, NOTICE, attribution and upstream provenance with the add-on.
