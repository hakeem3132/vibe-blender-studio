#!/usr/bin/env python3
"""Run the real Foundation acceptance against the packaged add-on."""
...
def main() -> int:
    ...
    if not archive.is_file():
        subprocess.run([sys.executable, str(ROOT / "scripts/build_addon.py")], cwd=ROOT, check=True)
    output = ROOT / "outputs/blender_real"
    output.mkdir(parents=True, exist_ok=True)
    home = ROOT / ".runtime/blender-home-foundation"
    environment = {
        **os.environ,
    ...
    blend = output / "foundation_acceptance.blend"
    result_path = output / "foundation_acceptance.json"
    diagnostics = output / "foundation_diagnostics.json"
    stage_one = _run(
        [
            str(blender),
            "--background",
            "--factory-startup",
            "--python",
            str(ROOT / "tests/blender_real/milestone_1_acceptance.py"),
            "--",
            str(archive),
            str(blend),
            str(result_path),
        ],
        output / "foundation_stage1.log",
        environment,
    )
    if stage_one != 0 or not result_path.is_file():
        print(f"Foundation stage one failed: exit {stage_one}")
        return 1
    stage_two = _run(
        [
            str(blender),
            ...
            str(blend),
            "--python",
            str(ROOT / "tests/blender_real/milestone_1_reopen.py"),
            "--",
            str(result_path),
            str(diagnostics),
        ],
        output / "foundation_stage2.log",
        environment,
    )
